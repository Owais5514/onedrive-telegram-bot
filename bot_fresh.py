#!/usr/bin/env python3
"""
OneDrive Telegram Bot - Fresh Setup Version
Optimized for new Azure app registration with proper application permissions
"""
...
class OneDriveTelegramBotFresh:
    def __init__(self):
    ...
    async def initialize_graph_client(self):
        ...
        try:
            ...
            # Test authentication and get users
            users = await self.graph_client.users.get()
            if users.value:
                # Cache users and pick the first one as default
                ...
                self.default_user_id = users.value[0].id
                self.authenticated = True
                
                logger.info(f"Graph client initialized. Found {len(users.value)} users.")
                logger.info(f"Default user: {users.value[0].display_name}")
                return True
            else:
    ...
    async def get_onedrive_items(self, path: str = "/", user_id: str = None) -> List[Dict[str, Any]]:
        ...
        try:
            if path == "/" or path == "":
                ...
                items = await self.graph_client.users.by_user_id(user_id).drive.root.item_with_path(clean_path).children.get()
            ...
            logger.info(f"Retrieved {len(result)} items from OneDrive path: {path} (user: {user_id})")
            return result
    ...
    async def get_folder_path_by_id(self, item_id: str, user_id: str = None) -> str:
        ...
        try:
            item = await self.graph_client.users.by_user_id(user_id).drive.items.by_drive_item_id(item_id).get()
            parent_path = "/"
            if item.parent_reference and item.parent_reference.path:
                parent_path = item.parent_reference.path.replace(f'/drive/root:', '') or "/"
    ...
    async def share_file(self, item_id: str, user_id: str = None) -> Optional[str]:
        ...
        try:
            # Get sharing link
            link = await self.graph_client.users.by_user_id(user_id).drive.items.by_drive_item_id(item_id).create_link.post({
                "type": "view",
                "scope": "anonymous"
            })
...
async def main():
    ...
    try:
        ...
        if auth_success:
            ...
            print("📖 Run 'python verify_fresh_setup.py' to diagnose issues")
        ...
        # Start the bot
        print("🚀 Starting OneDrive Telegram Bot (Fresh Setup)...")
